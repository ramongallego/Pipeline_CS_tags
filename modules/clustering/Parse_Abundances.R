...
sample_trans <- read_table(file.path(params$folder, "sample_trans.tmp"),show_col_types = F) 

samples |> 
  separate(files, into = c("fastq_header", "locus"), sep = "_Locus_", remove = F) |> 
...
if ( grepl ("yes", params$hash, ignore.case = TRUE)) {
  ...
  samples |> 
    rename(sequence=seq) |>
    inner_join(Hash_key, by ="sequence") |> 
